...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/peminjaman", { headers })
      .then((r) => r.json())
      .then((d) => {
        if (d.success) setRecent(d.data?.slice(0, 5) || []);
      });
  }, []);
    ...
    <div>
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500 text-sm mt-1">
          Berikut ringkasan kondisi peralatan laboratorium hari ini.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
        <StatCard
          ...
          value={stats?.totalPeralatan || 0}
          color="bg-blue-50 text-blue-600"
        />
        <StatCard
          icon={CheckCircle}
          label="Tersedia"
          value={stats?.peralatanTersedia || 0}
          color="bg-green-50 text-green-600"
        />
        <StatCard
          icon={TrendingUp}
          label="Sedang Dipinjam"
          value={stats?.peralatanDipinjam || 0}
          color="bg-orange-50 text-orange-600"
        />
        <StatCard
          icon={ClipboardList}
          label="Total Peminjaman"
          value={stats?.totalPeminjaman || 0}
          color="bg-purple-50 text-purple-600"
        />
        <StatCard
          icon={Clock}
          label="Menunggu Approval"
          value={stats?.peminjamanMenunggu || 0}
            ...
            <tbody>
              ...
              {recent.map((p) => (
                <tr key={p.id}>
                  ...
                  <td>
                    <span
                      className={`badge ${STATUS_COLORS[p.status] || 'bg-gray-100 text-gray-800'}`}
                    >
